// roulette/script.js - Motore di Gioco Roulette Live Pro
...
function init() {
    ...
    // Click sulle caselle per puntare
    document.querySelectorAll('.bet-spot').forEach(spot => {
        spot.addEventListener('click', () => {
            const currentBalance = getPraveBalance();
            if (isSpinning || currentBalance < selectedValue) return;

            // Scala dal saldo condiviso
            deductPraveBalance(selectedValue);
...
function startBettingTimer() {
    ...
    timerInterval = setInterval(() => {
        ...
        if (bettingTimeLeft <= 0) {
            clearInterval(timerInterval);
            timerBoxEl.classList.remove('timer-pulse');
            spinBall(); // Gira sempre, anche senza puntate
        }
    }, 1000);
}
...
function updateUI() {
    ...
    document.getElementById('balance').innerText = Math.floor(bal);
    let total = 0;
    for (let id in bets) total += bets[id].amount;
    document.getElementById('current-total-bet').innerText = total;
    refillBtn.style.display = (bal <= 0 && total <= 0) ? "block" : "none";
}
...
function manualReset() {
    if (isSpinning) return;
    for (let id in bets) addPraveBalance(bets[id].amount);
    bets = {};
    updateChips();
    updateUI();
    msgEl.innerText = "PUNTATE ANNULLATE";
    msgEl.className = "";
}
